...
const REQUIRED_ACCESS_GROUP_ERROR_MESSAGE = 'Access group is required!';
...
export const getImages = async (req: Request, res: Response) => {
  ...
  try {
    if (!accesGroup || accesGroup === '' || accesGroup === 'undefined') {
      return res
        .status(400)
        .json({ error: REQUIRED_ACCESS_GROUP_ERROR_MESSAGE });
    }
    ...
    if (!foundAccessGroup) {
      return res
        .status(404)
        .json({ error: ACCESS_GROUP_NOT_EXIST_ERROR_MESSAGE });
    }
